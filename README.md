...
- **File Upload Security**: Strict file type validation, size limits, and magic number verification
- **Input Validation**: Comprehensive validation via Pydantic models
- **Error Handling**: Secure error handling without information disclosure
- **CORS Configuration**: Configurable CORS settings for production environments
- **Optional VirusTotal Integration**: Cloud-based malware scanning (requires API key)

### Security Features

The application includes multiple layers of security:

1. **File Type Validation**: Only allows supported file types (PDF, PNG, JPG, JPEG, BMP, XLSX, XLS, CSV)
2. **File Size Limits**: Maximum 10MB file size to prevent resource exhaustion
3. **Magic Number Verification**: Validates file signatures to prevent spoofed files
4. **Heuristic Checks**: Blocks macro-enabled files and suspicious double extensions
5. **AI/ML Detection**: Framework for forgery and tampering detection (extensible)
6. **Optional VirusTotal**: Cloud-based malware scanning when API key is provided
