...
// Intercept fetch requests to handle API calls offline
self.addEventListener('fetch', event => {
    ...
    if (url.pathname.startsWith('/api/')) {
        if (url.pathname === '/api/protect' && event.request.method === 'POST') {
            ...
            event.respondWith(new Response(JSON.stringify({ error: 'API endpoint not supported offline' }), { status: 404, headers: { 'Content-Type': 'application/json' } }));
        }
        return;
    }
