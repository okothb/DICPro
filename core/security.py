...
MAX_FILE_SIZE_MB = 10
CLAMAV_SOCKET = '/var/run/clamav/clamd.ctl'  # Default for Linux; update as needed
...
def scan_with_clamav(file_path: str) -> Tuple[bool, str]:
    try:
        # Use clamscan as fallback if clamd is not available
        result = subprocess.run(['clamscan', '--no-summary', file_path], capture_output=True, text=True)
        if result.returncode == 0:
            return True, 'Clean (ClamAV)'
        else:
            return False, f'Infected (ClamAV): {result.stdout.strip()}'
...
def scan_file(file_path: str) -> Tuple[bool, str]:
    ...
    if not ok:
        return False, reason
    ok, reason = scan_with_clamav(file_path)
    if not ok:
        return False, reason
